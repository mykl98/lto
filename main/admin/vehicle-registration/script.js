$(document).ready(function() {
    setTimeout(function(){
        $("#vehicle-registration-menu").attr("href","#");
        $("#vehicle-registration-menu").addClass("active");
    },100)
})
...
function renderOwnerList(data){
    ...
    markUp = '<div class="form-group">\
                ...
                <select class="form-control" id="vehicle-owner">\
                    <option value="">SELECT OWNER</option>';
    lists.forEach(function(list){
        markUp += '<option value="'+list.idx+'">'+list.name+'</option>';
    })
    markUp += '</select></div>';
    $("#owner-select-container").html(markUp);
}
